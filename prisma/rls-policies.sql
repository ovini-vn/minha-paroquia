...
-- Fatia 7 (Catequese). Familia/dependentes + turmas/matricula/encontros/ritos.
...
-- Fatia 8 (Liturgia). Disponibilidade e escalas por celebracao.
...
-- Fatia 10 (Notificações). Só notifications recebe RLS — notification_preferences
-- não tem parish_id (é config do usuário, não dado de paróquia), mesmo padrão
-- de `users`/`sessions`.
...
-- Fatia 11 (Fechamento das lacunas de Pertencimento + Minha Comunidade).
-- Avisos — comunicados curtos da paróquia, distintos de Event e Post.
...
-- Fatia 14 (permission_overrides, P2 — ambiguidade #7). Precisa da mesma
-- cláusula "OR user_id" de parish_memberships: getSessionContext() lê os
-- próprios overrides dentro de withOwnMembershipLookup(), antes de o
-- contexto de parish_id existir.
