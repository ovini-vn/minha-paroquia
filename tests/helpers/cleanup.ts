...
export async function cleanupTenantData(params: { userIds?: string[]; parishIds?: string[] }): Promise<void> {
  ...
  await withPlatformContext(async (tx) => {
    if (parishIds.length > 0) {
      ...
      await tx.notificationDispatch.deleteMany({ where: { parishId: { in: parishIds } } });
      await tx.auditLog.deleteMany({ where: { parishId: { in: parishIds } } });
      ...
      await tx.recadoDoGrupo.deleteMany({ where: { parishId: { in: parishIds } } });
      await tx.presencaNoEncontro.deleteMany({ where: { parishId: { in: parishIds } } });
